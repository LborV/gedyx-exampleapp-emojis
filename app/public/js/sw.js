...
function hndlEventInstall(evt) {
    ...
    async function cacheStaticFiles() {
        const files = [
            ...
            // Kernel files
            '/kernel/Application.min.js',
            '/kernel/Controller.min.js',
            '/kernel/Parser.min.js',
            
            // main.js
            '/js/main.js',

            // Controllers
            'public/js/controllers/emojisController.min.js',

            // Emojis
            '/src/emojisList.min.js'
        ...
        await Promise.all(
            files.map(function (url) {
                return cacheStat.add(url).catch(function (reason) {
                    console.error(`'${url}' failed: ${String(reason)}`);
                });
            })
        );
    }
